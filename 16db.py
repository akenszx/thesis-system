# 1final.py (Updated with SQLite Database Integration)

# --- MODIFIED ---: Added sqlite3 import
...
# --- MODIFIED ---: Configuration updated for SQLite
CITY_MUNICIPALITY = "Echague"
PROVINCE = "Isabela" 
COUNTRY = "Philippines"
# Point this to your new SQLite database file.
# Make sure this file is in the same directory as your script.
DATABASE_PATH = "geocode_backup.db" 
CRIME_DATA_DB_PATH = "crime_data.db"
...
# --- The rest of your UI functions (inject_login_css, render_kpi_card, etc.) remain unchanged. ---
# --- For brevity, I'll skip to the data processing functions that need modification. ---
...
# --- MODIFIED ---: This function is now removed, as we use the database directly.
# def load_backup_data(file_path):
#    ...

# --- MODIFIED ---: Rewritten geocode function to use SQLite first.
def geocode_barangay(barangay_name, city, province, country="Philippines"):
    """
    Geocodes a barangay name, prioritizing lookup from a local SQLite database
    before falling back to the online Nominatim service.
    """
    brgy_name_clean = str(barangay_name).strip()
    
    # Step 1: Try to find coordinates in the local SQLite database first.
    try:
        conn = sqlite3.connect(DATABASE_PATH)
        cursor = conn.cursor()
        # Use a parameterized query to prevent SQL injection
        cursor.execute("SELECT Latitude, Longitude FROM barangays WHERE Barangay = ?", (brgy_name_clean,))
        result = cursor.fetchone()
        conn.close()
        
        if result and pd.notna(result[0]) and pd.notna(result[1]):
            # Found valid coordinates in the database, return them immediately.
            return result[0], result[1]
            
    except sqlite3.Error as e:
        # If the database file doesn't exist or there's an error, we'll just fall back to online geocoding.
        # You could add st.warning(f"Database lookup failed: {e}") here for debugging.
        pass

    # Step 2: If not found in the database, use the online geocoder as a fallback.
    time.sleep(1.2) # Keep the delay to avoid rate-limiting
    ...
    for query in queries: 
        try:
            ...
            if location:
                ...
                # Check if coordinates are within the expected region for Echague
                if 16.5 <= lat <= 16.9 and 121.5 <= lon <= 121.9:
                    return lat, lon
        except (ValueError, Exception):
            continue
            
    # Step 3: If all attempts fail, return None.
    return None, None

# --- MODIFIED ---: Simplified this function by removing the call to load backup data.
def batch_geocode_dataset(df):
    """Applies geocoding to unique Barangay names."""
    # The call to load_backup_data() is no longer needed.
    # The new geocode_barangay function handles the database logic internally.
...
def main_application():
    """Main crime mapping application with a new dashboard-style UI and persistent data storage."""

    def load_persistent_data():
        if os.path.exists(CRIME_DATA_DB_PATH):
            try:
                conn = sqlite3.connect(CRIME_DATA_DB_PATH)
                df = pd.read_sql("SELECT * FROM crime_records", conn)
                conn.close()
                ...
                if invalid_dates > 0:
                    st.warning(f"Note: Found and removed {invalid_dates} records from the database with an invalid 'Date_Occ' format.")
                    df.dropna(subset=['timestamp'], inplace=True)
                
                if df.empty:
                    st.error("After cleaning, no records with valid dates remain in the database.")
                    return pd.DataFrame()

                df['seconds'] = (df['timestamp'] - df['timestamp'].min()).dt.total_seconds()
                
                # ⭐ NEW: Check if data has been geocoded and clustered
                has_coordinates = 'LAT' in df.columns and 'LON' in df.columns
                has_clusters = 'cluster' in df.columns
                
                # Update clustering_complete flag based on database state
                if has_coordinates and has_clusters and not df['LAT'].isnull().all():
                ...
                st.error(f"Error loading persistent data: {e}")
                return pd.DataFrame()
        return pd.DataFrame()
    ...
    if 'df' not in st.session_state or st.session_state.df is None:
        ...
        if st.session_state.df is not None and not st.session_state.df.empty:
            status_msg = f"Loaded {len(st.session_state.df)} records from the database."
            # ⭐ NEW: Show if data is already processed
            if st.session_state.get('clustering_complete', False):
                status_msg += " (Already geocoded and clustered)"
            st.toast(status_msg)
...
# --- VIEW ROUTING ---
    if st.session_state.current_view == 'dashboard':
        ...
        with col_logout:
            st.container(border=False).button("➜] Logout", use_container_width=True, on_click=lambda: st.session_state.clear() or st.rerun())

        # Initialize reset confirmation state
        if 'show_reset_confirmation' not in st.session_state:
            st.session_state.show_reset_confirmation = False

        # First button - Shows confirmation
        if not st.session_state.show_reset_confirmation:
            if st.button("🚨 Reset All Crime Data", help="This deletes the persistent database and starts fresh. This action cannot be undone."):
                st.session_state.show_reset_confirmation = True
                st.rerun()
        
        # Confirmation section - Shows warning and confirm button
        if st.session_state.show_reset_confirmation:
            st.warning("⚠️ **WARNING:** You are about to delete ALL crime data from the system. This action is permanent and cannot be undone!")
            
            col1, col2, col3 = st.columns([1, 1, 2])
            with col1:
                if st.button("✅ Yes, Delete Everything", type="secondary", use_container_width=True):
                    if os.path.exists(CRIME_DATA_DB_PATH):
                        os.remove(CRIME_DATA_DB_PATH)
                    st.session_state.clear()
                    st.success("✅ All crime data has been permanently deleted. Upload a new file to begin. System will restart.")
                    time.sleep(4)
                    st.rerun()
        ...
        file = st.file_uploader(
            "Upload a new CSV file to add to the existing records.", 
            type=["csv"], 
            key="csv_uploader"  # ⭐ Added key for better control
        )
        
        if st.session_state.df is not None and not st.session_state.df.empty:
            st.info(f"✅ **Current Database Status:** {len(st.session_state.df)} total valid records loaded.")
        else:
            st.info("ℹ️ The crime database is currently empty. Please upload a file to start.")

        # ⭐ NEW: Check if file was just uploaded (prevent rerun loop)
        if file is not None and not st.session_state.file_uploaded:
            with st.spinner("Processing new file and updating database..."):
                new_df = pd.read_csv(file)
                st.write(f"Uploaded file contains {len(new_df)} new records.")

                required_cols = ['Date_Occ', 'Time_Occ', 'Crime_Type', 'Barangay']
                if not all(col in new_df.columns for col in required_cols):
                    st.error(f"❌ Missing Columns: Your file must contain {', '.join(required_cols)}.")
                    st.stop()
                
                original_new_count = len(new_df)
                new_df['timestamp'] = pd.to_datetime(new_df['Date_Occ'], errors='coerce')
                new_df.dropna(subset=['timestamp'], inplace=True)
                valid_new_count = len(new_df)
                
                rows_dropped = original_new_count - valid_new_count
                if rows_dropped > 0:
                    st.warning(f"⚠️ **Date Format Warning:** Removed {rows_dropped} rows from the uploaded file due to an invalid or missing date format in the 'Date_Occ' column. Please use a standard format like YYYY-MM-DD or MM/DD/YYYY.")

                if valid_new_count == 0:
                    st.error("❌ **Processing Stopped:** The uploaded file contains no records with a valid date format. Please fix the 'Date_Occ' column and re-upload.")
                    st.stop()
                
                existing_df = st.session_state.df if st.session_state.df is not None else pd.DataFrame()
                combined_df = pd.concat([existing_df, new_df.drop(columns=['timestamp'])], ignore_index=True)

                unique_incident_cols = ['Date_Occ', 'Time_Occ', 'Crime_Type', 'Barangay', 'Victim_Name', 'Suspect_Name']
                cols_to_use_for_dedup = [col for col in unique_incident_cols if col in combined_df.columns]
                
                if not cols_to_use_for_dedup:
                    st.warning("Could not find key columns to check for duplicates. Appending all data.")
                    final_df = combined_df
                else:
                    final_df = combined_df.drop_duplicates(subset=cols_to_use_for_dedup, keep='first')
                
                try:
                    conn = sqlite3.connect(CRIME_DATA_DB_PATH)
                    # ⭐ MODIFIED: Remove old analysis columns before saving
                    cols_to_save = [col for col in final_df.columns if col not in ['timestamp', 'seconds', 'LAT', 'LON', 'cluster']]
                    final_df[cols_to_save].to_sql('crime_records', conn, if_exists='replace', index=False)
                    conn.close()
                    st.success(f"✅ Database updated! Total records now: {len(final_df)}.")
                except Exception as e:
                    st.error(f"Failed to save data to database: {e}")
                    st.stop()

                # ⭐ NEW: Set flag to prevent rerun loop
                st.session_state.file_uploaded = True
                
                # Reload data and reset analysis flags
                st.session_state.df = load_persistent_data()
                st.session_state.clustering_complete = False
                st.session_state.df_clustered = None

                st.success("✅ New data uploaded successfully! Please run the analysis to process it.")
                st.balloons()
                time.sleep(5)
                st.rerun()
        
        # ⭐ NEW: Reset the flag when there's no file (user cleared it)
        elif file is None and st.session_state.file_uploaded:
            st.session_state.file_uploaded = False

        # --- UNIFIED ANALYSIS SECTION (Single Process) ---
        df = st.session_state.get('df', pd.DataFrame())
        if not df.empty:
            # ⭐ NEW: Check if analysis has already been completed
            is_analyzed = st.session_state.get('clustering_complete', False)
            
            if not is_analyzed:
                st.info("📋 **Data loaded.** Click the button below to process.")
                
                if st.button("🚀 **Start Complete Analysis**", 
                        use_container_width=True, 
                        type="primary",
                        help="This will geocode all barangays and run cluster analysis in one process."):
                    
                    current_df = df.copy()
                    
                    # Step 1: Check if geocoding is needed
                    needs_geocoding = 'LAT' not in current_df.columns or 'LON' not in current_df.columns or current_df['LAT'].isnull().any()
                    
                    if needs_geocoding:
                        with st.spinner("🌍 Step 1/2: Geocoding all barangays... This may take a moment."):
                            df_geocoded, removed, total_brgy, failed = batch_geocode_dataset(current_df)
                        ...
                        current_df = df_geocoded
                    else:
                        st.info("ℹ️ Coordinates already present. Skipping geocoding step.")
                    
                    # Step 2: Run clustering
                    with st.spinner("🔍 Step 2/2: Running CD-DBSCAN clustering analysis..."):
                        df_clustered, clusters_found, avg_query = run_cd_dbscan(current_df)
                    
                    # Step 3: Save results to both session state AND database
                    st.session_state.df = df_clustered.copy()
                    st.session_state.df_clustered = df_clustered.copy()
                    st.session_state.clustering_complete = True
                    ...
                    try:
                        conn = sqlite3.connect(CRIME_DATA_DB_PATH)
                        cols_to_save = [col for col in df_clustered.columns if col not in ['seconds', 'timestamp']]
                        df_clustered[cols_to_save].to_sql('crime_records', conn, if_exists='replace', index=False)
                        conn.close()
                        
                        st.success(f"✅ **Analysis Complete!** Found {clusters_found} crime clusters. Results saved to database.")
                        st.balloons()
                        time.sleep(2)
                        st.rerun()
                        
                    except Exception as e:
                        st.warning(f"⚠️ Clustering complete but couldn't save to database: {e}")
                        st.success("✅ Clustering complete! You can now access all features in the Main Menu below.")
                        st.balloons()
                        time.sleep(2)
                        st.rerun()
            else:
                # ⭐ NEW: Analysis already completed
                st.success(f"✅ **Data is ready!** {len(df)} records have been geocoded and clustered. Use the menu below to explore.")
                
                # Optional: Show re-analyze button
                with st.expander("🔄 Re-run Analysis (Optional)"):
                    st.warning("⚠️ This will re-process all data. Only use if you've made changes to the dataset.")
                    if st.button("Re-run Process", type="secondary"):
                        st.session_state.clustering_complete = False
                        st.rerun()

        st.header("Main Menu")
        is_disabled = not st.session_state.get('clustering_complete', False)
        disabled_tooltip = "Please run the analysis first to enable this feature."
        ...
        if st.button("← Back to Dashboard", use_container_width=True, help="Return to the main dashboard", key="back_to_dashboard"):
            st.session_state.current_view = 'dashboard'
            st.rerun()
        
        # --- START: THIS IS THE CRITICAL FIX ---

        # 1. Try to get the session-state-only dataframe
        df = st.session_state.get('df_clustered')
        
        # 2. If it's gone, fall back to the main df (loaded from the DB)
        if df is None or df.empty:
            df = st.session_state.get('df')
        
        # 3. If BOTH are empty, then stop
        if df is None or df.empty:
